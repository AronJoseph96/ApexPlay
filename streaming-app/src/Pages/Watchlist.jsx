...
export default function Watchlist() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setCollections(data);
      setActiveTab(data[data.length - 1]._id);
      setNewName("");
  ...
  const handleRemoveMovie = async (collectionId, movieId) => {
    try {
      const res  = await fetch(`${API}/users/${user._id}/collections/${collectionId}/movies/${movieId}`, { method: "DELETE" });
      const data = await res.json();
      setCollections(data);
  ...
  const handleDeleteCollection = async (collectionId) => {
    ...
    try {
      const res  = await fetch(`${API}/users/${user._id}/collections/${collectionId}`, { method: "DELETE" });
      const data = await res.json();
      setCollections(data);
      setActiveTab(data.length > 0 ? data[0]._id : null);
    } catch (e) { console.error(e); }
  };
            ...
            {activeCollection && (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  ...
                  <button
                    onClick={() => handleDeleteCollection(activeCollection._id)}
                    style={{
                    ...
                    {activeCollection.movies.map(movie => (
                      <div key={movie._id} style={{ width: 150 }}>
                        <div style={{ position: "relative" }}>
                          ...
                          />
                          <button
                            onClick={() => handleRemoveMovie(activeCollection._id, movie._id)}
                            title="Remove"
                            style={{
                              position: "absolute", top: 7, right: 7,
                              width: 26, height: 26, borderRadius: "50%",
                              background: "rgba(229,9,20,0.90)", color: "#fff",
